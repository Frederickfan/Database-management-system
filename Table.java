    ...
    public class RIDPageIterator implements BacktrackingIterator<RecordId> {
        ...
        */
        ...
        public boolean hasNext() {
            short k = cur;
            while (k < 8 * bitmap.length) {
                if (Bits.getBit(bitmap, k) == Bits.Bit.ONE) {
                    cur = k;
                    return true;
                }
                k++;
        ...
        public RecordId next() {
            while (Bits.getBit(bitmap, cur).equals(Bits.Bit.ZERO)) {
                cur++;
            }

            return new RecordId(pageNum, cur++);
        }

        public void mark() {
            if (this.cur!= 0 && this.cur >= this.mark) {
                mark = (short) (cur - 1);
            } else {
                return;
            }
