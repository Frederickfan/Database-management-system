...
public class SortOperator {
    ...
    public Run sortRun(Run run) throws DatabaseException {
        ...
        Iterator<Record> R_iter = run.iterator();
        List<Record> records = new ArrayList<>();

        while (R_iter.hasNext()) {
            records.add(R_iter.next());
        }

        records.sort(comparator);

        Run sorted_run = createRun();
        sorted_run.addRecords(records);
    ...
    public Run mergeSortedRuns(List<Run> runs) throws DatabaseException {
        ...
        while (i < runs.size()) {
            Run current_run = runs.get(i);
            Iterator<Record> current_iter = current_run.iterator();

            while (current_iter.hasNext()) {
                queue.add(new Pair<>(current_iter.next(), i));
            }

            i += 1;
        }

        Run mergedruns = createRun();

        while (queue.size() > 0) {
            List<DataBox> mergedvalues = queue.poll().getFirst().getValues();
            mergedruns.addRecord(mergedvalues);
        }
    ...
    public List<Run> mergePass(List<Run> runs) throws DatabaseException {
        List<Run> outcome = new ArrayList<>();
        int numBuffers = this.numBuffers - 1;

        int i = 0;
        while (i < runs.size()) {
            int edge = i + numBuffers;
            outcome.add(mergeSortedRuns(runs.subList(i, edge)));
            i += numBuffers;
        }
    ...
    public String sort() throws DatabaseException {
        List<Run> sortedR = new ArrayList<>();
        BacktrackingIterator<Page> pageIter = transaction.getPageIterator(tableName);
        pageIter.next();

        while (pageIter.hasNext()) {
            BacktrackingIterator<Record> recordIter = transaction.getBlockIterator(tableName, pageIter, numBuffers-1);
            Run unSortedrun = createRun();

            while (recordIter.hasNext()) {
                List<DataBox> nextRvalue = recordIter.next().getValues();
                unSortedrun.addRecord(nextRvalue);
            }

            Run sorted = sortRun(unSortedrun);
            sortedR.add(sorted);
        }

        while (sortedR.size() >= 2) {
            sortedR = mergePass(sortedR);
        }

        return sortedR.get(0).tableName();
